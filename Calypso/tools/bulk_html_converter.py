#!/usr/bin/env python3
"""
Convert all extracted chapters to HTML files.
Processes all chapter_XX folders and generates semantic HTML with linked CSS.
"""
...
def text_to_html(text_content, rules):
...
def convert_chapter_to_html(chapter_dir, output_dir, analysis_dir, chapter_id):
    ...
    if not os.path.exists(metadata_file) or not os.path.exists(content_file):
        print(f"  ✗ Missing files in {chapter_dir}")
        return False
    ...
    # Convert to HTML
    content_html = text_to_html(content_text, rules)
    ...
    if isinstance(chapter_id, str):
        filename = f"chapter_{chapter_id}.html"
    else:
        filename = f"chapter_{chapter_id:02d}.html"
...
def main():
    ...
    chapter_dirs = sorted(
        [
            (d, d.replace("chapter_", ""))
            for d in os.listdir(chapters_dir)
            if os.path.isdir(os.path.join(chapters_dir, d)) and d.startswith("chapter_")
        ]
    )
    ...
    print(f"Converting {len(chapter_dirs)} chapters to HTML...\n")

    converted = 0
    for chapter_folder, chapter_id in chapter_dirs:
        chapter_path = os.path.join(chapters_dir, chapter_folder)
        if convert_chapter_to_html(chapter_path, output_dir, analysis_dir, chapter_id):
            print(f"✓ {chapter_folder}")
            converted += 1
        else:
            print(f"✗ {chapter_folder}")

    print(f"\n✓ Successfully converted {converted}/{len(chapter_dirs)} chapters to HTML")
